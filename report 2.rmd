...
Polynomial Kernel: Moderate performance, slightly behind the linear kernel.
...
Interestingly, the radial kernel's performance is slightly better without PCA but is still very close with PCA (3 components). This indicates that PCA does a good job of maintaining the essential features for the radial kernel.
...
perform_decision_tree_analysis <- function(num_components) {
  pca_data <- pca_result$x[, 1:num_components]
  pca_df <- as.data.frame(pca_data)
  pca_df$hazardous <- factor(data$hazardous, levels = c("False", "True"))
  ...
  return(list(model = model_pca, cm = cm_pca))
}

# Run decision tree analysis with different number of PCA components
dt_results_2components <- perform_decision_tree_analysis(2)
dt_results_3components <- perform_decision_tree_analysis(3)
dt_results_4components <- perform_decision_tree_analysis(4)

# Combine results from different component runs
tree_results <- rbind(dt_results_2components,dt_results_3components, results_4components)
print(tree_results)
```
